      ...
      <Tooltip
        ...
        {(groupTarget === null || groupActionId === null) && (
          <div className="name-column" title={result.targetId ? `${result.targetId} › ${result.actionMnemonic}` : ""}>
            {/* bes-upload events don't have a target ID or file mnemonic. */}
            ...
            <div className="group">
              <div className="group-title action-id row">
                <div className="row action-label">
                  {/* Always render the target label when grouping by target or file. */}
                  <div>{group.results[0]?.targetId || "(Unknown target)"}</div>
                  {/* Then if grouping by file, show a chevron (">") followed by the file name. */}
                  {this.getGroupBy() === cache.GetCacheScoreCardRequest.GroupBy.GROUP_BY_ACTION && (
                    <>
                      <ChevronRight className="icon chevron" />
                      {/* If we have an file ID that looks like a digest, render it as a link
                          to the file page. */}
                      ...
                      {/* Otherwise render the mnemonic (if available) or the plaintext file ID,
                          which will be something like "bes-upload" or "remote-download".
                        */}
                      {!looksLikeDigest(group.actionId) && (
                        <div className="action-mnemonic">
                          {group.results[0]?.actionMnemonic || group.results[0]?.actionId || "(Unknown file)"}
                        </div>
                      )}
                    </>
                  )}
...
/**
 * Bazel includes some file IDs like "bes-upload" so we use this logic to try
