...
const (
	...
	// Allowed deadline extension for uploading file outputs.
	// The deadline of the original request may be extended by up to this amount
	// in order to give enough time to upload file outputs.
...
// configuration error that will prevent the file from executing properly,
// even if retried.
...
func (s *Executor) ExecuteTaskAndStreamResults(ctx context.Context, st *repb.ScheduledTask, stream operation.StreamLike) (retry bool, err error) {
	...
	if !req.GetSkipCacheLookup() {
		log.CtxInfof(ctx, "Checking file cache for existing result.")
		...
		if err == nil {
			...
			log.CtxInfof(ctx, "Found existing result in file cache, all done.")
			return false, nil
		}
	}
	...
	// If the file failed or do_not_cache is set, upload information about the error via a failed
	...
	if task.GetAction().GetDoNotCache() || cmdResult.Error != nil || cmdResult.ExitCode != 0 {
		...
		if err != nil {
			return finishWithErrFn(status.UnavailableErrorf("Error uploading file result: %s", err.Error()))
		}
