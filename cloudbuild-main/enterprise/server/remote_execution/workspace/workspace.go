...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/dirtools"
...
// Workspace holds the working tree for an file and keeps track of
// inputs and outputs.
...
// DownloadInputs downloads any missing inputs for the current file.
...
func (ws *Workspace) Clean() error {
	...
	if ws.Opts.Preserve {
		...
		for _, outputDirPath := range cmd.GetOutputDirectories() {
			...
			// output directory.
			// TODO: This nested loop impl may slow down the file if there are a lot
