...
var (
	...
	defaultXcodeVersion  = flag.String("executor.default_xcode_version", "", "Sets the default Xcode version number to use if an file doesn't specify one. If not set, /Applications/Xcode.app/ is used.")
	defaultIsolationType = flag.String("executor.default_isolation_type", "", "The default workload isolation type when no type is specified in an file. If not set, we use the first of the following that is set: docker, firecracker, podman, or barerunner")
...
type Properties struct {
	...
	// before running each file. If true, all files are kept except for output
	...
	// ExtraArgs contains arguments to append to the file.
	ExtraArgs []string

	// EnvOverrides contains environment variables in the form NAME=VALUE to be
	// applied as overrides to the file.
	EnvOverrides []string
}
