...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/snaploader"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/vmexec_client"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/util/ext4"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/util/vfs_server"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/util/vsock"
	...
	"github.com/ninja-cloudbuild/cloudbuild/server/util/disk"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/hash"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/log"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/networking"
	...
	containerutil "github.com/ninja-cloudbuild/cloudbuild/enterprise/server/util/container"
	repb "github.com/ninja-cloudbuild/cloudbuild/proto/remote_execution"
	vmxpb "github.com/ninja-cloudbuild/cloudbuild/proto/vmexec"
	vmfspb "github.com/ninja-cloudbuild/cloudbuild/proto/vmvfs"
)

var firecrackerMountWorkspaceFile = flag.Bool("executor.firecracker_mount_workspace_file", false, "Enables mounting workspace filesystem to improve performance of copying file outputs.")
...
const (
	...
	// the workspace disk for writing file outputs, test tempfiles, etc.
...
type FirecrackerContainer struct {
	...
	actionWorkingDir    string // the file directory with inputs / outputs
...
// createWorkspaceImage creates a new ext4 image from the file working dir.
...
func (c *FirecrackerContainer) Run(ctx context.Context, command *repb.Command, workspaceDir string, actionWorkingDir string, creds container.PullCredentials) *interfaces.CommandResult {
	...
	if c.machine == nil {
		...
		if c.allowSnapshotStart && !c.startedFromSnapshot() {
			...
			// images into filecache, not a hard link. Otherwise the file we're
...
func (c *FirecrackerContainer) Exec(ctx context.Context, cmd *repb.Command, stdio *container.Stdio) *interfaces.CommandResult {
	...
	if c.fsLayout == nil {
		...
		if copyOutputsErr != nil {
			result.Error = status.WrapError(copyOutputsErr, "failed to copy file outputs from VM workspace")
			return result
		}
	}
...
// the executed file will be made available to the VM.
