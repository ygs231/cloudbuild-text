...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/commandutil"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/container"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/containers/bare"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/containers/docker"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/containers/firecracker"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/containers/podman"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/containers/sandbox"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/platform"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/vfs"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/workspace"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/tasksize"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/util/vfs_server"
	"github.com/ninja-cloudbuild/cloudbuild/server/environment"
	"github.com/ninja-cloudbuild/cloudbuild/server/interfaces"
	"github.com/ninja-cloudbuild/cloudbuild/server/metrics"
	"github.com/ninja-cloudbuild/cloudbuild/server/remote_cache/cachetools"
	"github.com/ninja-cloudbuild/cloudbuild/server/remote_cache/digest"
	"github.com/ninja-cloudbuild/cloudbuild/server/resources"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/alert"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/background"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/flagutil"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/lockingbuffer"
	...
	aclpb "github.com/ninja-cloudbuild/cloudbuild/proto/acl"
	...
	uidpb "github.com/ninja-cloudbuild/cloudbuild/proto/user_id"
	vfspb "github.com/ninja-cloudbuild/cloudbuild/proto/vfs"
	wkpb "github.com/ninja-cloudbuild/cloudbuild/proto/worker"
)
...
var (
	...
	dockerNetwork           = flag.String("executor.docker_network", "", "If set, set docker/podman --network to this value by default. Can be overridden per-file with the `dockerNetwork` exec property, which accepts values 'off' (--network=none) or 'bridge' (--network=<default>).")
	dockerCapAdd            = flag.String("docker_cap_add", "", "Sets --cap-add= on the docker command. Comma separated.")
	dockerSiblingContainers = flag.Bool("executor.docker_sibling_containers", false, "If set, mount the configured Docker socket to containers spawned for each file, to enable Docker-out-of-Docker (DooD). Takes effect only if docker_socket is also set. Should not be set by executors that can run untrusted code.")
...
func (r *commandRunner) PrepareForTask(ctx context.Context) error {
	...
	// those paths were written as read-only inputs in a previous file.
