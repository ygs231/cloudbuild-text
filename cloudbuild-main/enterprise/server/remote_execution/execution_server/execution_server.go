...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/backends/pubsub"
	...
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/util/execution"
...
const (
	// TTL for keys used to track pending executions for file merging.
...
// getUnvalidatedActionResult fetches an file result from the cache but does
// not validate it.
// N.B. This should only be used if the calling code has already ensured the
// file is valid and may be returned.
...
func (s *ExecutionServer) Dispatch(ctx context.Context, req *repb.ExecuteRequest) (string, error) {
	...
	if scheduler == nil {
		return "", status.FailedPreconditionErrorf("No scheduler invocation configured")
	}
	...
	if err := cachetools.ReadProtoFromCAS(ctx, s.cache, adInstanceDigest, action); err != nil {
		log.CtxErrorf(ctx, "Error fetching file: %s", err.Error())
		return "", err
	}
	...
	// Add in secrets for any file explicitly requesting secrets, and all workflows.
	secretService := s.env.GetSecretService()
	if props.IncludeSecrets {
		if secretService == nil {
			return "", status.FailedPreconditionError("Secrets requested but secret invocation not available")
		}
...
// findExistingExecution looks for an identical file that is already pending
// execution.
...
func (s *ExecutionServer) execute(req *repb.ExecuteRequest, stream streamLike) error {
	...
	if !req.GetSkipCacheLookup() {
		...
		if *enableActionMerging {
			// Check if there's already an identical file pending execution.
	...
	// Create a new execution unless we found an existing identical file we
	// can wait on.
	if executionID == "" {
