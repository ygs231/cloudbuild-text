...
var (
	...
	triggerEvent       = flag.String("trigger_event", "", "Event type that triggered the file runner.")
	...
	targetBranch       = flag.String("target_branch", "", "Branch to check file triggers against.")
	...
	actionName         = flag.String("action_name", "", "If set, run the specified file and *only* that file, ignoring trigger conditions.")
	invocationID       = flag.String("invocation_id", "", "If set, use the specified invocation ID for the workflow file. Ignored if action_name is not set.")
	...
	debug             = flag.Bool("debug", false, "Print additional debug information in the file logs.")
...
type workspace struct {
	...
	// specified in the file config, but the CI runner itself will stay in
	// "repo-root".
	rootDir string
	...
	// reported for all file logs instead of actually executing the file.
	setupError error
...
func (r *buildEventReporter) Stop(exitCode int, exitCodeName string) error {
	...
	if err := r.bep.Finish(); err != nil {
		...
		// that the executor can retry the file, so that we have another chance.
...
func (r *buildEventReporter) nextProgressEvent() (*bespb.BuildEvent, error) {
	...
	if err != nil {
		return nil, status.WrapError(err, "failed to read file logs")
	}
	if len(buf) == 0 {
		return nil, nil
	}
...
func run() error {
	...
	if *bazelSubCommand != "" {
		...
		// If a specific file was specified, filter to configured
		// actions with a matching file name.
		action, err = findAction(cfg.Actions, *actionName)
		if err != nil {
			return err
		}
	} else {
		return status.InvalidArgumentError("One of --file or --bazel_sub_command must be specified.")
	}
...
// RunAction runs the specified file and streams the progress to the invocation via the buildEventReporter.
func (ws *workspace) RunAction(ctx context.Context, action *config.Action, buildEventReporter *buildEventReporter) (*actionResult, error) {
	...
	// and the logs uploaded to BuildBuddy for this file. Anything that we want
...
func (ar *actionRunner) Run(ctx context.Context, ws *workspace) error {
	ar.reporter.Printf("Running file %q", ar.action.Name)
...
func findAction(actions []*config.Action, name string) (*config.Action, error) {
	...
	return nil, status.NotFoundErrorf("file %q not found", name)
}
...
func actionDebugString(action *config.Action) string {
	...
	if err != nil {
		return fmt.Sprintf("<failed to marshal file: %s>", err)
	}
