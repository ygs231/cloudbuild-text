...
type ExecutorOptions struct {
	...
	// To ease debugging, you should specify an explicit name if it's important to the test on which executor an file
	// is executed.
	Name string
...
func (r *Env) DownloadOutputsToNewTempDir(res *CommandResult) string {
	...
	if err := r.rbeClient.DownloadActionOutputs(context.Background(), env, res.CommandResult, tmpDir); err != nil {
		assert.FailNow(r.t, "failed to download file outputs", err.Error())
	}
	return tmpDir
}
...
func (c *Command) MustFailToStart() error {
	result := c.getResult()
	require.Nil(c.env.t, result.ActionResult, "command unexpectedly return an file result")
...
func (c *Command) MustTerminateAbnormally() *CommandResult {
	...
	require.NotNil(
		...
		"expected file result with debug outputs to help diagnose abnormal termination")
...
// invocation. An ExecuteResponse is not made available to clients in this case.
...
type ExecuteOpts struct {
	...
	InvocationID string
	// ActionTimeout is the value of Timeout to be passed to the spawned file.
	ActionTimeout time.Duration
	// Whether file cache should be checked for existing results. By default,
	// we skip the file cache check for tests.
	CheckCache bool
}
...
func (r *Env) Execute(command *repb.Command, opts *ExecuteOpts) *Command {
	...
	if err != nil {
		assert.FailNowf(r.t, fmt.Sprintf("unable to request file execution for command %q", name), err.Error())
	}
