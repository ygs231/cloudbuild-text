...
func (c *Command) Start(ctx context.Context, opts *StartOpts) error {
	...
	log.Debugf("Executing command %q with file digest %s", c.Name, c.actionResourceName.GetDigest().GetHash())
	...
	if err != nil {
		cancel()
		return status.UnknownErrorf("unable to request file execution for command %q: %s", c.Name, err)
	}
...
func (c *Client) PrepareCommand(ctx context.Context, instanceName string, name string, inputRootDigest *repb.Digest, commandProto *repb.Command, timeout time.Duration) (*Command, error) {
	...
	if err != nil {
		return nil, status.UnknownErrorf("unable to upload file for command %q to CAS: %s", name, err)
	}
