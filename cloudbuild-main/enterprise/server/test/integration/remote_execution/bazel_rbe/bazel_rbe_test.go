...
// invocation using Bazel as the client.
...
func TestSimpleAction_TerminateWithSIGABRT(t *testing.T) {
	...
	res := runRemoteShellActionViaBazel(
		t, ctx, env,
		"echo >&2 'Aborting test file'; kill -ABRT $$")

	assert.Contains(
		t, res.Stderr, "Aborting test file",
...
func runRemoteShellActionViaBazel(t *testing.T, ctx context.Context, env *rbetest.Env, shCommand string, extraBazelArgs ...string) *bazel.InvocationResult {
	ws := testbazel.MakeTempWorkspace(t, map[string]string{
		"WORKSPACE": "",
		// Define a bazel rule that runs exactly one file, which creates a dummy
		...
		// also wind up executing more than one file, so test assertions are less
		// intuitive.
		"defs.bzl": `
def _exec_impl(ctx):
...
`,
	...
	// Execute just the test file remotely.
	buildArgs := []string{
		":exec",
