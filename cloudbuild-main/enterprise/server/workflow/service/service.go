...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/operation"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/remote_execution/platform"
	"github.com/ninja-cloudbuild/cloudbuild/enterprise/server/workflow/config"
	"github.com/ninja-cloudbuild/cloudbuild/server/backends/github"
	"github.com/ninja-cloudbuild/cloudbuild/server/endpoint_urls/build_buddy_url"
	"github.com/ninja-cloudbuild/cloudbuild/server/endpoint_urls/cache_api_url"
	"github.com/ninja-cloudbuild/cloudbuild/server/endpoint_urls/events_api_url"
	"github.com/ninja-cloudbuild/cloudbuild/server/endpoint_urls/remote_exec_api_url"
	...
	"github.com/ninja-cloudbuild/cloudbuild/server/util/bazel_request"
	...
	"github.com/ninja-cloudbuild/cloudbuild/server/util/random"
	...
	ctxpb "github.com/ninja-cloudbuild/cloudbuild/proto/context"
	...
	wfpb "github.com/ninja-cloudbuild/cloudbuild/proto/workflow"
...
var (
	...
	// ApprovalRequired is an error indicating that a workflow file could not be
	// run at a commit because it is untrusted. An approving review at the
	// commit will allow the file to run.
...
func instanceName(wf *tables.Workflow, wd *interfaces.WebhookData, workflowActionName string, gitCleanExclude []string) string {
	// Use a unique remote instance name per repo URL and workflow file name, to help
...
func (ws *workflowService) ExecuteWorkflow(ctx context.Context, req *wfpb.ExecuteWorkflowRequest) (*wfpb.ExecuteWorkflowResponse, error) {
	...
	// Fetch the workflow config for the file we're about to execute
	...
	if action == nil {
		return nil, status.NotFoundErrorf("Workflow file %q not found", req.GetActionName())
	}
...
// Creates an file that executes the CI runner for the given workflow and params.
// Returns the digest of the file as well as the invocation ID that the CI runner
...
func (ws *workflowService) createActionForWorkflow(ctx context.Context, wf *tables.Workflow, wd *interfaces.WebhookData, isTrusted bool, ak *tables.APIKey, instanceName string, workflowAction *config.Action, invocationID string, extraArgs []string) (*repb.Digest, error) {
	...
	if os == "" || os == platform.LinuxOperatingSystemName {
		...
		if *enableFirecracker {
			...
			// to the executor between file runs.
...
func (ws *workflowService) startWorkflow(webhookID string, r *http.Request) error {
	...
	// for each file matching the webhook event.
	cfg, err := ws.fetchWorkflowConfig(ctx, gitProvider, wf, wd)
	if err != nil {
		return err
	}
	for _, action := range cfg.Actions {
		...
		if err != nil {
			if err == ApprovalRequired {
				log.Infof("Skipping workflow file %s (%s) %q (requires approval)", wf.WorkflowID, wf.RepoURL, action.Name)
				if err := ws.createApprovalRequiredStatus(ctx, wf, wd, action.Name); err != nil {
					log.Warningf("Failed to create workflow %s (%s) file status: %s", wf.WorkflowID, wf.RepoURL, err)
				}
				continue
			}
			// TODO: Create a UI for these errors instead of just logging on the
			// server.
			log.Warningf("Failed to execute workflow %s (%s) file %q: %s", wf.WorkflowID, wf.RepoURL, action.Name, err)
		}
	}
	return nil
}
