...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/server/util/git"
...
// createAction creates and uploads an file that will trigger the CI runner
// to checkout the specified repo and execute the specified bazel file,
...
func (r *runnerService) Run(ctx context.Context, req *rnpb.RunRequest) (*rnpb.RunResponse, error) {
	...
	log.Debugf("Uploaded runner file to cache. Digest: %s/%d", actionDigest.GetHash(), actionDigest.GetSizeBytes())
