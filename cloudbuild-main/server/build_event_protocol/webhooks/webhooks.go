...
var (
	...
	gcsCredentialsJSON = flag.String("integrations.invocation_upload.gcs_credentials", "", "Credentials JSON for the Google invocation account used to authenticate when GCS is used as the invocation upload target. ** Enterprise only **")
)
...
// this returns a token source that generates tokens for the configured invocation
