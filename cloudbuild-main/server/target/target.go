...
import (
	...
	"encoding/base64"
	"fmt"
	cmpb "github.com/ninja-cloudbuild/cloudbuild/proto/api/v1/common"
	...
	tppb "github.com/ninja-cloudbuild/cloudbuild/proto/target_pagination"
		...
		for rows.Next() {
			...
			if _, ok := seenTargets[targetID]; !ok {
				...
				targets = append(targets, &trpb.Target{
					...
					TargetType: common.TargetType(cmpb.TargetType(row.TargetType)),
					TestSize:   common.TestSize(cmpb.TestSize(row.TestSize)),
				})
			}
			...
			statuses[targetID] = append(statuses[targetID], &trpb.TargetStatus{
				...
				Status:       common.Status(convertToCommonStatus(build_event_stream.TestStatus(row.Status))),
