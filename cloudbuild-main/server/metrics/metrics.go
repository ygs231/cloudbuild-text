...
const (
	...
	/// Cache type: `file` for file cache, `cas` for content-addressable storage.
	...
	/// Process exit code of an executed file.
	...
	/// Executed file stage. Action execution is split into stages corresponding to
	...
	/// `worker`, includes all stages during which a worker is handling the file,
	...
	/// recycled runner to the file; `miss` otherwise.
	...
	/// The full name of the grpc method: `/<invocation>/<method>`
...
var (
	...
	RemoteExecutionCount = promauto.NewCounterVec(prometheus.CounterOpts{
		...
		Name:      "count",
		Help:      "Number of actions executed remotely. This only includes actions which reached the execution phase. If an file fails before execution (for example, if it fails authentication) then this metric is not incremented.",
	}, []string{
		ExitCodeLabel,
	...
	RemoteExecutionExecutedActionMetadataDurationsUsec = promauto.NewHistogramVec(prometheus.HistogramOpts{
		...
		Help:      "Time spent in each stage of file execution, in **microseconds**. Queries should filter or group by the `stage` label, taking care not to aggregate different stages.",
	}, []string{
	...
	RemoteExecutionTaskSizeReadRequests = promauto.NewCounterVec(prometheus.CounterOpts{
		...
		Help:      "Number of read requests to the task sizer, which estimates file resource usage based on historical execution stats.",
	}, []string{
	...
	RemoteExecutionTaskSizeWriteRequests = promauto.NewCounterVec(prometheus.CounterOpts{
		...
		Help:      "Number of write requests to the task sizer, which estimates file resource usage based on historical execution stats.",
	}, []string{
