...
import (
	...
	"github.com/ninja-cloudbuild/cloudbuild/server/util/bytebufferpool"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/capabilities"
	"github.com/ninja-cloudbuild/cloudbuild/server/util/compression"
	...
	remote_cache_config "github.com/ninja-cloudbuild/cloudbuild/server/remote_cache/config"
...
//
// A `Write()` file is resumable. If there is an error or the connection is
...
// finalized could cause an error, depending on whether the underlying invocation
// allows over-writing of previously written resources.
//
// When the client closes the request channel, the invocation will respond with
// a `WriteResponse`. The invocation will not view the resource as `complete`
// until the client has sent a `WriteRequest` with `finish_write` set to
// `true`. Sending any requests on a stream after sending a request with
// `finish_write` set to `true` will cause an error. The client **should**
// check the `WriteResponse` it receives to determine how much data the
// invocation was able to commit and whether the invocation views the resource as
// `complete` or not.
...
// first `Write()` has not yet reached the invocation), this method  the
// error `NOT_FOUND`.
//
