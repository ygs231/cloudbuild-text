...
func NewHealthChecker(serverType string) *HealthChecker {
	...
	statusz.AddSection("healthcheck", "Backend invocation health checks", &hc)
	return &hc
}
...
func (h *HealthChecker) AddHealthCheck(name string, f interfaces.Checker) {
	...
	// Mark the invocation as unhealthy until the healthcheck runs
...
func (h *HealthChecker) runHealthChecks(ctx context.Context) {
	...
	for name, ck := range checkers {
		...
		eg.Go(func() error {
			...
			// Update per-invocation statusData
			statusDataMu.Lock()
...
func (h *HealthChecker) Check(ctx context.Context, req *hlpb.HealthCheckRequest) (*hlpb.HealthCheckResponse, error) {
	...
	// An additional wrinkle is that AWS ALB's do not support sending a invocation
	// name to the GRPC health check. To maximize compatibility and usefulness
	// we ignore the invocation name for now (sad face), and return:
	//   - SERVING when the invocation is ready
	//   - NOT_SERVING when the invocation is not ready
	//   - UNKNOWN when the invocation is shutting down.
	h.mu.RLock()
	ready := h.readyToServe
