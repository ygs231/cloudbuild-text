...
var (
	...
	invocationID = flag.String("invocation_id", "", "Invocation ID. This is required when fetching the result of a failed file. Otherwise, it's optional.")
...
//
// Show a failed file result proto (requires invocation ID):
//
...
func main() {
	...
	// Handle ActionResults (these are stored in the file cache)
	if *blobType == "ActionResult" {
		ar, err := cachetools.GetActionResult(ctx, acClient, ind)
		if err != nil {
			log.Infof("Could not fetch ActionResult; maybe the file failed. Attempting to fetch failed file using invocation ID = %q", *invocationID)
